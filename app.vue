...
<template>
  <NuxtExample class="h-50" dir="advanced/use-cookie">
    <template v-if="user">
      <h1 class="tet-3xl mb-3">Welcome, {{ user.name }}</h1>
      <div>
        <UAlert title="Logged-In" color="primary" icon="i-heroicons-light-bulb">
          <template #description>You have logged in <b>{{ logins }} times</b>!</template>
        </UAlert>
      </div>
      <div class="mt-3">
        <UButton color="orange" icon="i-heroicons-arrow-left" @click="logout">Logout</UButton>
      </div>
    </template>
    <template v-else>
